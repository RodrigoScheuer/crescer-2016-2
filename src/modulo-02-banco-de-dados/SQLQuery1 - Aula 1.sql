...
select NomeEmpregado, DataAdmissao, datediff(MONTH, convert(char(10), DataAdmissao, 103), '31/12/2000') as meses_Trabalhados
from Empregado 
where convert(char(10), DataAdmissao, 103) between '01/05/1980' and '20/01/1982'; 
-- TEM ERRO (Busca ano 1983)


-- selecionar cargo com maior quantidade de funcionarios
select Cargo, count(NomeEmpregado) as qtd_funcionarios from Empregado
group by Cargo;		-- falta selecionar so um elemento
...
select Nome, UF, COUNT(1) as qtd_registros from Cidade
group by Nome, UF
having count(1) > 1;
